...
function Gallery() {
  ...
  return <main className="gallery-page section-shell">
    ...
    <div className="gallery-toolbar"><div className="filter-row" aria-label="Arbeiten nach Stylistin filtern">{filters.map(tab => <button key={tab.id} className={active === tab.id ? "active" : ""} aria-pressed={active === tab.id} onClick={() => { setSelected(null); router.replace(tab.id === "all" ? "/galerie" : `/galerie?stylist=${tab.id}`, { scroll: false }); }}>{tab.label}</button>)}</div><span className="gallery-count" aria-live="polite">{items.length} Arbeiten</span></div>
    <div className="gallery-grid">{items.map((item, i) => <button className="gallery-item" key={item.src} onPointerEnter={() => { void prepareViewer(item.src).catch(() => undefined); }} onFocus={() => { void prepareViewer(item.src).catch(() => undefined); }} onClick={() => setSelected(i)} aria-label={`${item.caption} von ${item.stylistName} vergrößern`}><ClipReveal className="gallery-photo"><FadeImage src={item.src} alt={item.caption} fill sizes="(max-width: 600px) 90vw, (max-width: 900px) 45vw, 30vw" /><span className="image-arrow"><Expand size={18} /></span></ClipReveal><span className="gallery-caption"><span>{item.caption}</span><span>von {item.stylistName}</span></span></button>)}</div>
    {!items.length && <div className="gallery-empty"><h2>Weitere Looks folgen.</h2><p>Von {TEAM.find(member => member.slug === active)?.name ?? "unserem Team"} sind aktuell noch keine Arbeiten in der Galerie. Lerne uns persönlich im Salon kennen.</p><Link href="/booking" className="text-link">Termin finden <ArrowUpRight size={17} /></Link></div>}
    <div className="gallery-end"><p>Deinen nächsten Lieblingslook schon im Kopf?</p><Link href="/booking" className="button button-primary">Wir freuen uns auf dich <ArrowUpRight size={17} /></Link></div>
