  ...
  return (
    <main className="booking-page section-shell">
      <header className="page-intro">
        <FadeIn delay={0.08}><p className="eyebrow">Dein nächster Besuch</p></FadeIn>
        <FadeIn delay={0.2}><h1>Ein bisschen Zeit.<br /><em>Nur für dich.</em></h1></FadeIn>
        <FadeIn delay={0.34}><p>Wähle deinen Salon, deine Behandlung und deinen Wunschtermin. Wir freuen uns auf ein Wiedersehen – oder ein erstes Kennenlernen.</p></FadeIn>
      </header>
      <nav className="booking-locations" aria-label="Salon für die Buchung wählen">
        {Object.entries(LOCATIONS).map(([key, salon], i) => (
          <FadeIn key={key} delay={0.18 + i * 0.1}>
            <Link href={`/booking?location=${key}`} scroll={false} className={active === key ? "active" : ""} aria-current={active === key ? "page" : undefined}>
              <span>Dresden</span>{salon.name}<span>{salon.street}</span><ArrowUpRight size={23} />
            </Link>
          </FadeIn>
        ))}
      </nav>
      <div className="booking-layout">
        <aside className="booking-sidebar">
          <FadeIn delay={0.08}>
            <p className="eyebrow">Dein Salon</p>
            <h2>{loc.name}</h2>
          </FadeIn>
          <FadeIn delay={0.18}>
