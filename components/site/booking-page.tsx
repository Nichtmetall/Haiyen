  ...
  return (
    <main className="booking-page section-shell">
      <header className="page-intro">
        <FadeIn delay={0.05}><p className="eyebrow">Dein nächster Besuch</p></FadeIn>
        <FadeIn delay={0.16}><h1>Ein bisschen Zeit.<br /><em>Nur für dich.</em></h1></FadeIn>
        <FadeIn delay={0.28}>
          <p>Wähle deinen Salon, deine Behandlung und deinen Wunschtermin. Wir freuen uns auf ein Wiedersehen – oder ein erstes Kennenlernen.</p>
        </FadeIn>
      </header>
        ...
        <section aria-label={`Online-Terminbuchung Dresden ${loc.name}`} className="booking-widget-section">
          <FadeIn delay={0.2}>
            <div className="booking-widget-label">
              <span><CalendarCheck size={17} /> Dein Termin in {loc.name}</span>
              <span>Online buchen mit Planity</span>
            </div>
            <div className="booking-widget" key={active}>
              <ConsentEmbed actionLabel="Buchungssystem laden" allow="payment" className="h-full w-full" description="Mit dem Laden stimmst du der Übertragung von Daten an Planity zu." heading="Bereit für deinen nächsten Besuch?" icon={<CalendarCheck aria-hidden="true" className="h-9 w-9" />} id="myIframe" src={loc.bookingUrl} title={`Online-Terminbuchung – Salon Dresden ${loc.name}`} tone="dark" />
            </div>
          </FadeIn>
        </section>
      </div>
