...
import { FAQS, GALLERY_ITEMS, TEAM } from "./data";
...
export const HomePageContent = () => (
  <main>
    ...
    <section className="section-shell selected-work">
      ...
      <div className="work-grid">{[GALLERY_ITEMS[3], GALLERY_ITEMS[1], GALLERY_ITEMS[8]].map((item, i) => <FadeUp key={item.src} delay={i * 0.08}><Link className="work-item" href={`/galerie?stylist=${item.stylist}`}><ScrollImage className="work-image"><Image src={item.src} alt={item.caption} fill sizes="(max-width: 760px) 85vw, 30vw" /><span className="image-arrow"><ArrowUpRight size={20} /></span></ScrollImage><div className="work-caption"><span>{item.caption}</span><span>von {item.stylistName.split(" ")[0]}</span></div></Link></FadeUp>)}</div>
    </section>
    <ServicesSection />
    <section className="section-shell home-team" id="team">
      ...
      <div className="team-preview">{TEAM.map((member, i) => <ClipReveal key={member.slug} delay={i * 0.1}><Link href={`/team#${member.slug}`} className="team-preview-person"><div className="portrait"><Image src={member.img} alt={member.name} fill sizes="(max-width: 600px) 45vw, 23vw" /></div><h3>{member.name}</h3><p>{member.role}</p></Link></ClipReveal>)}</div>
    </section>
    ...
    <section id="standorte" className="section-shell locations-section">
      ...
      <div className="locations-grid">{Object.entries(LOCATIONS).map(([key, loc], i) => <FadeUp className="location-block" key={key} delay={i * 0.1}><div className="location-heading"><span className="eyebrow">Dresden / 0{i + 1}</span><h3>{loc.name}</h3></div><SalonPhoto slot={key as LocationKey} className="location-salon-photo" /><div className="location-details"><div><a href={loc.mapUrl} target="_blank" rel="noreferrer">{loc.street}<br />{loc.city}</a><a className="location-phone" href={`tel:${loc.phoneHref}`}>{loc.phone}</a></div><dl>{loc.hours.map(row => <div key={row.days}><dt>{row.days}</dt><dd>{row.time}</dd></div>)}</dl></div><Link className="button button-green" href={`/booking?location=${key}`}>Termin in {loc.name} <ArrowUpRight size={17} /></Link><details className="map-details"><summary>Anfahrt & Karte <Plus size={16} /></summary><div className="location-map"><ConsentEmbed src={loc.mapEmbed} title={`Google Maps – Haiyen Hairdesign ${loc.name}`} fallbackHref={loc.mapUrl} fallbackLabel="Route öffnen" /></div></details></FadeUp>)}</div>
    </section>
