  ...
  <main>
    ...
    <section id="salon" className="section-shell salon-story" aria-labelledby="salon-title">
      ...
      <div className="salon-story-images">
        <FadeIn delay={0.1}><SalonPhoto slot="styling" className="salon-story-styling" /></FadeIn>
        <FadeIn delay={0.22}><SalonPhoto slot="care" className="salon-story-care" /></FadeIn>
      </div>
    </section>
    <section className="section-shell selected-work section-dark">
      <FadeIn className="section-heading">
        <div><p className="eyebrow">Von unseren Händen. Für deinen Alltag.</p><h2>So vielfältig<br />wie <em>du.</em></h2></div>
        <div className="section-aside"><p>Weiche Übergänge, lebendige Farben und ein Schnitt, der zu dir gehört. Ein kleiner Einblick in unsere Arbeit.</p><Link className="text-link" href="/galerie">Alle Arbeiten entdecken <ArrowUpRight size={17} /></Link></div>
      </FadeIn>
      <div className="work-grid">{[GALLERY_ITEMS[3], GALLERY_ITEMS[1], GALLERY_ITEMS[8]].map((item, i) => (
        <FadeIn key={item.src} delay={i * 0.12}>
          <Link className="work-item" href={`/galerie?stylist=${item.stylist}`}>
            <ScrollImage className="work-image"><FadeImage src={item.src} alt={item.caption} fill sizes="(max-width: 760px) 85vw, 30vw" /><span className="image-arrow"><ArrowUpRight size={20} /></span></ScrollImage>
            <div className="work-caption"><span>{item.caption}</span><span>von {item.stylistName.split(" ")[0]}</span></div>
          </Link>
        </FadeIn>
      ))}</div>
    </section>
    ...
    <section className="section-shell home-team section-dark" id="team">
      <FadeIn className="section-heading">
        <div><p className="eyebrow">Persönlich für dich da</p><h2>Dein <em>Team.</em></h2></div>
        <Link className="text-link" href="/team">Lerne unser Team kennen <ArrowUpRight size={17} /></Link>
      </FadeIn>
      <div className="home-team-intro">
        <FadeIn delay={0.1} className="team-group-photo"><FadeImage src={TEAM_IMAGE.src} alt={TEAM_IMAGE.alt} width={1800} height={1800} sizes="(max-width: 760px) 88vw, 44vw" /></FadeIn>
        <FadeIn delay={0.22}>
      ...
      <FadeIn delay={0.16}><TeamPreviewCarousel /></FadeIn>
    </section>
    ...
    <section id="standorte" className="section-shell locations-section section-dark">
      <FadeIn className="section-heading">
        <div><p className="eyebrow">Zweimal Dresden. Ein vertrautes Gefühl.</p><h2>Wir freuen uns<br /><em>auf dich.</em></h2></div>
        <p className="section-aside">Dein nächster Besuch beginnt hier.<br />Wähle deinen Salon und finde deinen Wunschtermin.</p>
      </FadeIn>
      <div className="locations-grid">{Object.entries(LOCATIONS).map(([key, loc], i) => (
        <FadeIn className="location-block" key={key} delay={i * 0.14}>
          <div id={key} className="location-heading"><span className="eyebrow">Dresden / 0{i + 1}</span><h3>Friseur in<br />Dresden {loc.name}</h3></div>
          <div className="location-details"><div><a href={loc.mapUrl} target="_blank" rel="noreferrer">{loc.street}<br />{loc.city}</a><a className="location-phone" href={`tel:${loc.phoneHref}`}>{loc.phone}</a></div><dl>{loc.hours.map(row => <div key={row.days}><dt>{row.days}</dt><dd>{row.time}</dd></div>)}</dl></div>
          <Link className="button button-primary" href={`/booking?location=${key}`}>Termin in {loc.name} <ArrowUpRight size={17} /></Link>
          <details className="map-details"><summary>Anfahrt & Karte <Plus size={16} /></summary><div className="location-map"><ConsentEmbed src={loc.mapEmbed} title={`Google Maps – Haiyen Hairdesign ${loc.name}`} fallbackHref={loc.mapUrl} fallbackLabel="Route öffnen" /></div></details>
        </FadeIn>
      ))}</div>
    </section>
    <section className="section-shell faq-section">
      <FadeIn>
        <div><p className="eyebrow">Vor deinem Besuch</p><h2>Noch eine<br /><em>Frage?</em></h2></div>
      </FadeIn>
      <div className="faq-list">{FAQS.map((faq, i) => (
        <FadeIn key={faq.q} delay={i * 0.07}>
          <details><summary>{faq.q}<Plus size={18} /></summary><p>{faq.a}</p></details>
        </FadeIn>
      ))}</div>
    </section>
  </main>
