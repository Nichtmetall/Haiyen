...
export default function SiteChrome({ children }: { children: React.ReactNode }) {
  ...
  return <MotionConfig reducedMotion="user"><SmoothScroll><div className="site-wrap">
    ...
    <dialog className="mobile-menu" ref={menu} aria-label="Navigation" data-lenis-prevent><div className="mobile-menu-top"><span>Haiyen Hairdesign</span><button aria-label="Menü schließen" onClick={closeMenu}><X size={25} /></button></div><nav>{links.map(link => <Link key={link.href} href={link.href} onClick={closeMenu}>{link.label}<ArrowUpRight size={22} /></Link>)}</nav><p className="eyebrow light">Ihr nächster Besuch</p>{Object.entries(LOCATIONS).map(([key, loc]) => <Link className="button button-gold" key={key} onClick={closeMenu} href={`/booking?location=${key}`}>Termin in {loc.name}<ArrowUpRight size={18} /></Link>)}</dialog>
    <div id="main-content" tabIndex={-1}>{children}</div>
    {!legal && <section className="contact-section"><div className="section-shell contact-layout"><FadeIn delay={0.05}><div><p className="eyebrow">Wir sind für Sie da</p><h2>Was liegt Ihnen<br /><em>auf dem Herzen?</em></h2><a className="text-link" href="mailto:info@haiyen-hairdesign.de">info@haiyen-hairdesign.de <ArrowUpRight size={16} /></a><ContactSocialLinks /></div></FadeIn><FadeIn delay={0.18}><details className="contact-details"><summary>Schreiben Sie uns eine Nachricht <Plus size={21} /></summary><form onSubmit={contact}><div className="form-grid"><label>Ihr Name<input name="name" autoComplete="name" required placeholder="Vor- und Nachname" /></label><label>Ihre E-Mail-Adresse<input name="email" type="email" autoComplete="email" required placeholder="name@beispiel.de" /></label><label>Telefonnummer <span>(optional)</span><input name="phone" type="tel" autoComplete="tel" /></label><label>Ihr Salon<select name="location"><option>Dresden Striesen</option><option>Dresden Neustadt</option></select></label></div><label>Ihre Nachricht<textarea name="message" required rows={4} placeholder="Wie können wir Ihnen helfen?" /></label><p>Wir bereiten die Nachricht in Ihrem E-Mail-Programm vor. Ihre Angaben werden hier nicht gespeichert. Mehr dazu im <Link href="/datenschutz">Datenschutz</Link>.</p><button type="submit" className="button button-primary">E-Mail vorbereiten <ArrowUpRight size={17} /></button></form></details></FadeIn></div></section>}
    <footer className="site-footer"><div className="section-shell"><div className="footer-main"><FadeIn delay={0.05} className="footer-brand"><Link href="/" aria-label="Haiyen Hairdesign – Startseite"><Image src="/images/logos/haiyen_logo_hell.png" alt="Haiyen Hairdesign" width={808} height={246} /></Link><p>Ihr Haar. Ihr Stil.<br />Ihr vertrauter Ort.</p></FadeIn><FadeIn delay={0.12}><nav aria-label="Weitere Seiten"><p className="eyebrow light">Bei Haiyen</p><Link href="/team">Das Team</Link><Link href="/galerie">Unsere Arbeiten</Link><Link href="/#leistungen">Leistungen & Preise</Link><Link href="/booking">Termin buchen</Link></nav></FadeIn>{Object.entries(LOCATIONS).map(([key, loc], i) => <FadeIn className="footer-location" key={key} delay={0.18 + i * 0.08}><p className="eyebrow light">Dresden {loc.name}</p><a href={loc.mapUrl} target="_blank" rel="noreferrer">{loc.street}<br />{loc.city}</a><a href={`tel:${loc.phoneHref}`}>{loc.phone}</a><Link href={`/booking?location=${key}`}>Termin in {loc.name}</Link></FadeIn>)}</div><FadeIn delay={0.32}><div className="footer-bottom"><span>© 2026 Haiyen Hairdesign</span><div><Link href="/impressum">Impressum</Link><Link href="/datenschutz">Datenschutz</Link><button onClick={openSettings}>Cookie-Einstellungen</button></div><span>Mit Gefühl. In Dresden.</span></div><p className="footer-disclaimer">Einige Bilder und Texte auf dieser Website wurden mit KI-Unterstützung erstellt.</p></FadeIn></div></footer>
    {!booking && !legal && <MobileBookingLink key={pathname} />}
  </div></SmoothScroll></MotionConfig>;
}
