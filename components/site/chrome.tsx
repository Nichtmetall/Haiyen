...
export default function SiteChrome({ children }: { children: React.ReactNode }) {
  ...
  return <MotionConfig reducedMotion="user"><div className="site-wrap">
    ...
    {!legal && <section className="contact-section"><div className="section-shell contact-layout"><div><FadeIn delay={0.05}><p className="eyebrow">Wir sind für dich da</p></FadeIn><FadeIn delay={0.14}><h2>Was liegt dir<br /><em>auf dem Herzen?</em></h2></FadeIn><FadeIn delay={0.24}><a className="text-link" href="mailto:info@haiyen-hairdesign.de">info@haiyen-hairdesign.de <ArrowUpRight size={16} /></a></FadeIn><FadeIn delay={0.34}><ContactSocialLinks /></FadeIn></div><FadeIn delay={0.18}><details className="contact-details"><summary>Schreib uns eine Nachricht <Plus size={21} /></summary><form onSubmit={contact}><div className="form-grid"><label>Dein Name<input name="name" autoComplete="name" required placeholder="Vor- und Nachname" /></label><label>Deine E-Mail-Adresse<input name="email" type="email" autoComplete="email" required placeholder="name@beispiel.de" /></label><label>Telefonnummer <span>(optional)</span><input name="phone" type="tel" autoComplete="tel" /></label><label>Dein Salon<select name="location"><option>Dresden Striesen</option><option>Dresden Neustadt</option></select></label></div><label>Deine Nachricht<textarea name="message" required rows={4} placeholder="Wie können wir dir helfen?" /></label><p>Wir bereiten die Nachricht in deinem E-Mail-Programm vor. Deine Angaben werden hier nicht gespeichert. Mehr dazu im <Link href="/datenschutz">Datenschutz</Link>.</p><button type="submit" className="button button-primary">E-Mail vorbereiten <ArrowUpRight size={17} /></button></form></details></FadeIn></div></section>}
    <footer className="site-footer"><div className="section-shell"><div className="footer-main"><FadeIn className="footer-brand"><Link href="/" aria-label="Haiyen Hairdesign – Startseite"><Image src="/images/logos/haiyen_logo_hell.png" alt="Haiyen Hairdesign" width={808} height={246} /></Link><p>Dein Haar. Dein Stil.<br />Dein vertrauter Ort.</p></FadeIn><FadeIn delay={0.1}><nav aria-label="Weitere Seiten"><p className="eyebrow light">Bei Haiyen</p><Link href="/team">Das Team</Link><Link href="/galerie">Unsere Arbeiten</Link><Link href="/#leistungen">Leistungen & Preise</Link><Link href="/booking">Termin buchen</Link></nav></FadeIn>{Object.entries(LOCATIONS).map(([key, loc], i) => <FadeIn className="footer-location" delay={0.18 + i * 0.08} key={key}><p className="eyebrow light">Dresden {loc.name}</p><a href={loc.mapUrl} target="_blank" rel="noreferrer">{loc.street}<br />{loc.city}</a><a href={`tel:${loc.phoneHref}`}>{loc.phone}</a><Link href={`/booking?location=${key}`}>Termin in {loc.name}</Link></FadeIn>)}</div><FadeIn delay={0.12} className="footer-bottom"><span>© 2026 Haiyen Hairdesign</span><div><Link href="/impressum">Impressum</Link><Link href="/datenschutz">Datenschutz</Link><button onClick={openSettings}>Cookie-Einstellungen</button></div><span>Mit Gefühl. In Dresden.</span></FadeIn></div></footer>
